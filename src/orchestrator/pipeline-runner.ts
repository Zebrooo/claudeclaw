...
export const AND_GATES: Record<
  string,
  { requires: string[]; dispatches: string[] }
> = {
  developer_start: {
...
export class PipelineRunner {
  ...
  private async dispatchToAgent(
    ...
    if (!jid) {
      logger.warn(
        { agentFolder, eventId: event.id },
        'No registered JID for agent — skipping',
      );
      return;
    }
    ...
    logger.info(
      { agentFolder, eventId: event.id, type: event.type },
      'Dispatched to agent',
    );
  }
  ...
  async tick(): Promise<void> {
    ...
    for (const event of this.bus.getPendingEvents()) {
      ...
      if (gateName) {
        const gate = AND_GATES[gateName];
        if (!gate) continue;
        if (!this.bus.allTypesPresent(event.task_id, gate.requires)) continue;
        const gateKey = `${event.task_id}:${gateName}`;
        if (firedGates.has(gateKey)) {
          // Gate already fired this tick — just mark this contributing event dispatched
          this.bus.markDispatched(event.id);
          continue;
        }
        firedGates.add(gateKey);
        this.bus.markDispatched(event.id);
        for (const folder of gate.dispatches) {
          await this.dispatchToAgent(event, folder);
        }
        continue;
      }
      const targets = SUBSCRIPTIONS[event.type] ?? [];
      if (targets.length === 0) {
  ...
  start(): void {
    ...
    const loop = async () => {
      ...
      try {
        ...
        logger.error({ err }, 'PipelineRunner tick error');
      }
