...
describe('AND_GATES', () => {
  it('developer_start requires architect_approved and design_ready', () => {
    expect(AND_GATES['developer_start'].requires).toContain(
      'architect_approved',
    );
...
describe('PipelineRunner.getTargetAgents', () => {
  ...
  it('returns developer when both architect_approved and design_ready published', () => {
    ...
    const evt = bus
      .getPendingEvents()
      .find((e) => e.type === 'architect_approved')!;
    const targets = runner.getTargetAgents(evt);
    expect(targets).toContain('pipeline_developer');
  });
});

describe('PipelineRunner.tick', () => {
  let bus: EventBus;
  ...
  beforeEach(() => {
    _initTestDatabase();
    bus = new EventBus();
    mockRoute = vi.fn().mockResolvedValue(undefined);
    runner = new PipelineRunner(
      bus,
      {
        route: mockRoute,
  ...
  it('dispatches developer only once when both architect_approved and design_ready are pending', async () => {
    const taskId = bus.createTask('build feature');
    bus.publish({
      type: 'architect_approved',
      source_agent: 'architect',
      task_id: taskId,
